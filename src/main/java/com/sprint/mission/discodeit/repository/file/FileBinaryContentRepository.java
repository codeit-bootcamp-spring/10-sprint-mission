...
public class FileBinaryContentRepository implements BinaryContentRepository {
    ...
    private void init() {
        if (!Files.exists(dirPath)) {
            try {
                ...
                throw new RuntimeException("바이너리 콘텐츠 데이터 폴더 생성 실패", e);
            }
        }
    }
    ...
    public Optional<BinaryContent> findById(UUID binaryContentId) {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(Files.newInputStream(path))) {
            ...
            throw new RuntimeException("바이너리 콘텐츠 파일 조회 실패", e);
        }
    }
    ...
    public List<BinaryContent> findAll() {
        ...
        try (Stream<Path> stream = Files.list(dirPath)) {
            ...
            throw new RuntimeException("바이너리 콘텐츠 파일 목록 조회 실패", e);
        }
    }
    ...
    public void delete(BinaryContent binaryContent) {
        ...
        try {
            ...
            throw new RuntimeException("바이너리 콘텐츠 데이터 삭제 실패", e);
        }
    }
    ...
    private void writeToFile(BinaryContent binaryContent) {
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(Files.newOutputStream(path))) {
            ...
            throw new RuntimeException("바이너리 콘텐츠 데이터 저장 실패", e);
        }
    }
}
