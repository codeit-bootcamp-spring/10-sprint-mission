...
//    }
    ...
    public static String viewFileMessage(Channel channel, MessageService messageService) {
        StringBuilder sb = new StringBuilder();
        sb.append("[").append(channel.getChannelName()).append(" 채팅방]").append("\n");
...
//    }
    ...
    // 서버 전체 메시지 출력
    public static String viewAllFileMessages(FileMessageRepository messageRepository) {
        String allMessages = messageRepository.findAllMessage().stream()
                .map(Message::toString)
                .collect(Collectors.joining("\n"));
        return allMessages.isEmpty() ? "(메시지 없음)" : allMessages;
    }
