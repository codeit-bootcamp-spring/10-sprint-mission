...
public class DiscodeitApplication {
	...
	public static void main(String[] args) {
		...
		UserService userService = context.getBean(JCFUserService.class);
		...
		// 1. 등록
		UserResponse user1 = userService.createAccount(new UserCreateRequest("김철수", "철수", "kcs@example.com", "1234", null));
		UserResponse user2 = userService.createAccount(new UserCreateRequest("김영희", "0희", "kyh@example.com", "0000", null));
		UserResponse user3 = userService.createAccount(new UserCreateRequest("홍길동", "길동이", "hkd@example.com", "hkd9876", null));
		System.out.println("[등록] user1, user2, user3 계정 생성 완료");

		// 2. 조회 (단건, 다건)
		System.out.println("[단건 조회]: " + userService.getAccountById(user1.id()).name());
		System.out.println("[다건 조회] 전체 유저 수: " + userService.getAllAccounts().size());

		// 3. 수정
		userService.updateAccount(user1.id(), new UserUpdateRequest(user1.id(), "고철수", null, "go@example.com", null));
		System.out.println("[수정] user1 계정 수정 완료");

		// 수정된 데이터 조회
		UserResponse updatedUser = userService.getAccountById(user1.id());
		System.out.println("[수정 데이터 확인]");
		System.out.println("- 이름: " + updatedUser.name());
		System.out.println("- 닉네임: " + updatedUser.nickname());
		System.out.println("- 이메일: " + updatedUser.email());
		System.out.println("- 수정시간: " + updatedUser.updatedAt());
		...
		// 6. 조회를 통해 삭제 확인
		try {
			userService.getAccountById(user1.id());
