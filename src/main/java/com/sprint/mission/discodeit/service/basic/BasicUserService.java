...
public class BasicUserService implements UserService {
    ...
    @Override
    public UserResponseDTO create(UserCreateRequestDTO req, BinaryContentDTO profileDto) {
        ...
        if (userRepository.findAll()
            ...
            throw new IllegalStateException("같은 email 또는 username를 사용하는 User가 이미 존재함");
        }
        ...
        if (profileDto != null) {
            ...
            BinaryContent binaryContent = new BinaryContent(profileDto.contentType(),
                profileDto.bytes());
        ...
        User user = userDTOMapper.regtoUser(req,
            profileId); // DTO -> Entity를 통해 생성 요청 DTO에서 유저 객체를 생성한다.
        ...
        UserStatus savedUserStatus = userStatusRepository.save(
            new UserStatus(savedUser.getId())); // UserStatus 객체를 새로 만들고 레포의 save를 통해 영속화
    ...
    public List<UserResponseDTO> findAll() {
        return userRepository
            ...
            .map(u -> {
                return userDTOMapper.userToResponse(u, userStatusRepository.findByUserId(u.getId())
    ...
    @Override
    public UserResponseDTO update(UUID userId, UserUpdateDTO req, BinaryContentDTO profileDto) {
...
//        }
        ...
        if (profileDto != null) {
            ...
            BinaryContent saved = binaryContentRepository.save(
                binaryContent); // 해당 BinaryContent를 영속화함.
            newProfileId = saved.getId(); // newProfileId에 영속화된 객체의 id를 담음.
        }

        // 유저 도메인 객체의 update 메소드를 통해 업데이트.
        user.update(req.newUsername(), req.newEmail(), req.newPassword(), newProfileId);
        ...
        return userDTOMapper.userToResponse(savedUser,
            userStatusRepository.findByUserId(savedUser.getId()).orElseThrow(
                () -> new NoSuchElementException(
                    "해당 UserStatus가 존재하지 않습니다!"))); // Entities -> DTO 후 리
    }
