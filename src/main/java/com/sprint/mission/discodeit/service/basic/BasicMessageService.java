...
public class BasicMessageService implements MessageService {
    ...
    public MessageResponse createMessage(MessageCreateRequest request) {
        ...
        // 채널이 존재하는지 검증
        channelRepository.findChannelById(request.channelId())
                .orElseThrow(() -> new RuntimeException("채널이 존재하지 않습니다."));

        // 유저가 존재하는지 검증
        userRepository.findUserById(request.authorId())
    ...
    public MessageResponse updateMessage(MessageUpdateRequest request) {
        ...
        // 수정 대상 메시지가 존재하는지 검증
        Message message = messageRepository.findMessageByMessageId(request.messageId())
