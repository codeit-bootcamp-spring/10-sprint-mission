...
public class BasicNotificationService implements NotificationService {
    ...
    if (TransactionSynchronizationManager.isSynchronizationActive()) {
      TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
        ...
        public void afterCommit() {
          sendNotificationEvents(savedNotifications);
        }
      });
    } else {
      sendNotificationEvents(savedNotifications);
  ...
  private void sendNotificationEvents(List<Notification> notifications) {
    for (Notification notification : notifications) {
      try {
        NotificationDto dto = notificationMapper.toDto(notification);
        sseService.send(List.of(notification.getReceiverId()), "notifications.created", dto);
      } catch (Exception e) {
        log.error("Failed to send SSE notification to receiver {}", notification.getReceiverId(), e);
      }
    }
  }
