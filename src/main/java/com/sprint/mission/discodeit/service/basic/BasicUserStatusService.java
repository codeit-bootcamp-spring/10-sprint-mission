...
public class BasicUserStatusService implements UserStatusService {
    ...
    // User 정보를 포함하여 반환
    @Override
    public UserResponseDto updateByUserId(UUID userId, UserStatusUpdateByUserIdDto userStatusUpdateByUserIdDto) {
        UserStatus userStatus = userStatusRepository.findByUserId(userId)
                .orElseThrow(() -> new IllegalArgumentException("해당 사용자의 상태정보가 없습니다."));
        User user = userRepository.findById(userId).orElseThrow(() -> new IllegalArgumentException("해당 사용자가 없습ㄴ디ㅏ."));
        ...
        System.out.println(updatedStatus.getStatusType());
