...
public class JavaApplication {
    ...
    public static void main(String[] args) {
        ...
        //채널 등록 테스트
        Channel channel1 = channelService.createChannel("공부 채널");
        Channel channel2 = channelService.createChannel("코딩 채널");
        ...
        //채널 수정 테스트
        Channel updatedChannel = channelService.updateChannel(channel1.getId(), "게임 채널");
        ...
        //유저 등록 테스트
        User user1 = userService.createUser("이규빈", "hello@hello.com");
        User user2 = userService.createUser("홍길동", "hi@codeit.com");
        ...
        //유저 이메일 변경 테스트
        User updatedEmailUser = userService.updateUser(user1.getId(), null, "bye@bye.com");
        System.out.println("유저 이메일 변경 = " + updatedEmailUser);

        //유저 이름 변경 테스트
        User updatedNameUser = userService.updateUser(user1.getId(), "김코딩", null);
        System.out.println("유저 이름 변경 = " + updatedNameUser);

        //유저 이메일, 이름 모두 변경 테스트
        User updatedUser = userService.updateUser(user1.getId(), "이공부", "good@good.com");
        ...
        channelService.createChannel("테스트 채널");
        Channel testChannel = channelService.findChannelByName("테스트 채널");

        channelService.createChannel("관리자 채널");
        Channel adminChannel = channelService.findChannelByName("관리자 채널");

        userService.createUser("관리자 A", "adminA@discodeit.com");
        User adminA = userService.findUserByEmail("adminA@discodeit.com");

        userService.createUser("관리자 B", "adminB@discodeit.com");
        User adminB = userService.findUserByEmail("adminB@discodeit.com");

        //관리자 A가 테스트 채널에 메세지 2번, 관리자 채널에 메세지 1번 전송
        Message message1 = messageService.send("첫번째 메세지 전송 테스트입니다.", adminA.getId(), testChannel.getId());

        Message message2 = messageService.send("두번째 메세지 전송 테스트입니다.", adminA.getId(), testChannel.getId());

        Message message3 = messageService.send("여기는 관리자 채널입니다.", adminA.getId(), adminChannel.getId());

        //메세지 Id로 메세지 조회
        Message foundMessage = messageService.getMessage(message3.getId());
        System.out.println("메세지 ID로 찾은 메세지 = " + foundMessage);

        //특정 채널의 특정 유저의 메세지 조회 - 테스트 채널, 관리자 A
        System.out.println("테스트 채널의 관리자 A의 모든 채팅");
        List<Message> testChannelAdminAMessages = messageService.findMessagesByUserAndChannel(adminA.getId(), testChannel.getId());
        ...
        messageService.send("관리자 B가 테스트 합니다.", adminB.getId(), testChannel.getId());
        ...
        Message updated = messageService.updateMessage(messageId, "첫번째 메세지를 수정해봤습니다.");
        for (Message testChannelAdminAMessage : testChannelAdminAMessages) {
            System.out.println(testChannelAdminAMessage);
        }
        System.out.println();
        ...
        User joinUser = userService.createUser("관리자 C", "adminC@discodeit.com");
        channelService.joinChannel(joinUser.getId(), testChannel.getId());
        ...
        channelService.leaveChannel(joinUser.getId(), testChannel.getId());
        ...
        userService.deleteUser(adminA.getId());
        ...
        System.out.println(testChannel.getMessages());
        System.out.println(testChannel.getUsers());
        channelService.deleteChannel(testChannel.getId());
